...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] = {
    val value = client.linksFrom(of).value.flatMap {
      case Right(a) => WikiResult.traverse(a.toSeq)(client.nameOfArticle).value
      case Left(e) => Future.successful(Left(e))
    }

    val strSeq = value.flatMap {
      case Right(s) => Future.successful(Right(s.toSet))
      case Left(e) => Future.successful(Left(e))
    }
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      ...
      var (head, tail) = q.dequeue

      if (head._1 > maxDepth) then
        return WikiResult.successful(None)

      val adjPages: Future[Either[Seq[WikiError], Set[ArticleId]]] = client.linksFrom(head._2).value

      val result: Future[Either[Seq[WikiError], Option[Int]]] = adjPages.flatMap {
        case Right(artIdSeq) =>
          if artIdSeq.contains(target) then
            Future.successful(Right(Some(head._1)))
          else
            artIdSeq.foreach { artId =>
              if !visited.contains(artId) then
                tail = tail.enqueue(head._1 + 1 -> artId)
            }
            iter(visited ++ artIdSeq, tail).value

        case Left(e) => Future.successful(Left(e))
      }

      WikiResult(result).fallbackTo(iter(visited, tail))
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    ...
    val res: List[Future[Either[Seq[WikiError], (String, String, Option[Int])]]] =
      titles.flatMap { title1 =>
        titles.collect { case title2
          if title1 != title2 =>

            val distance: Future[Either[Seq[WikiError], Option[Int]]] =
              client.searchId(title1).zip(client.searchId(title2)).value.flatMap {
                      case Right((articleId1, articleId2)) => breadthFirstSearch(articleId1, articleId2, maxDepth).value
                      case Left(e) => Future.successful(Left(e))
              }

            distance.flatMap {
              case Right(dist) => Future.successful(Right((title1, title2, dist)))
              case Left(e) => Future.successful(Left(e))
            }
        }
      }
    WikiResult.traverse(res)(a => WikiResult(a))
