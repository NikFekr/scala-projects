...
case class WikiResult[A](value: Future[Either[Seq[WikiError], A]]):
  ...
  def map[B](f: A => B)(using ExecutionContext): WikiResult[B] =
    val m : Future[Either[Seq[WikiError], B]] = this.value.map {
      case Right(value1) => Right(f(value1))
      case Left(e) => Left(e)
    }
    WikiResult(m)
...
object WikiResult:
  ...
  def traverse[A, B](as: Seq[A])(f: A => WikiResult[B])(using ExecutionContext): WikiResult[Seq[B]] =
    as.foldLeft[WikiResult[Seq[B]]](WikiResult(Future.successful(Right(Vector.empty)))){
      (wikiResultBs, a) =>
        val wikiResultB: WikiResult[B] = f(a)
        wikiResultBs.zip(wikiResultB).map((bs, b) => bs :+ b)
    }
