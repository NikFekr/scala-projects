...
object PersistentModel extends Model:
  ...
  def create(task: Task): Id =
    val id = IdGenerator(loadId()).nextId().next
    val tasks = loadTasks()
    var updatedTasks = (id -> task) :: tasks.tasks.toList
    updatedTasks = updatedTasks.sortBy((id, _) => id)
  ...
  def update(id: Id)(f: Task => Task): Option[Task] = {
    val tasks = loadTasks()
    var updatedTasks = tasks.tasks.toList.filter((id1, _) => id1 != id)
    val maybeTask = read(id).map(f)
    updatedTasks = (id -> maybeTask.get) :: updatedTasks
    saveTasks(Tasks(updatedTasks))
    maybeTask
  }
  ...
  def delete(id: Id): Boolean =
    val tasks = loadTasks()
    val boolean = read(id).isDefined
    val  updatedTasks = tasks.tasks.toList.filter((id1, _) => id1 != id)
    saveTasks(Tasks(updatedTasks))
    boolean
  ...
  def tasks(tag: Tag): Tasks =
    Tasks(tasks.tasks.filter(task => task._2.tags.contains(tag)))

  def complete(id: Id): Option[Task] =
    update(id)(task => task.complete)

  def tags: Tags =
    Tags(tasks.tasks.flatMap(task => task._2.tags).toSet.toList)
