...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val deleteTwoMinAndFindMin: Boolean =
        if findMin(deleteMin(deleteMin(meldedHeap))) == max then true else false
      ...
      val insertMinAndFindMin: Boolean =
        if findMin(insert(min, meldedHeap)) == min then true else false
  ...
  val meldingHeaps: (String, Prop) =
    ...
    def check (heap1: List[Node], heap2: List[Node], meldedHeap: List[Node]): Boolean = (heap1, heap2) match
    case (Nil , Nil) => true
    case (heap1, Nil) =>
      if findMin(heap1) != findMin(meldedHeap) then
        false
      else
          check(deleteMin(heap1), Nil, deleteMin(meldedHeap))
    case (Nil, heap2) =>
      if findMin(heap2) != findMin(meldedHeap) then
        false
      else
        check(Nil, deleteMin(heap2), deleteMin(meldedHeap))
    case (heap1, heap2) =>
      if findMin(heap1) == findMin(meldedHeap) then
        check(deleteMin(heap1), heap2, deleteMin(meldedHeap))
      else if findMin(heap2) == findMin(meldedHeap) then
    ...
    forAll { (heap1: List[Node], heap2: List[Node]) =>
      if heap1.isEmpty && heap2.isEmpty then true else check(heap1, heap2, meld(heap1, heap2))
    }
