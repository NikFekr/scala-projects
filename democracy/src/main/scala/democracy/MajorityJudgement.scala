...
object Grade:
  ...
  def median(grades: Seq[Grade]): Grade = {
    val sortedGrades = grades.sortBy(grade => grade.ordinal)
...
case class Election(description: String, candidates: Set[Candidate]):
  ...
  def elect(ballots: Seq[Ballot]): Candidate =
    ...
    ballots.flatMap(ballot => {
      ballot.grades.map((candidate, grade) => candidate -> grade).toSeq
    })
    ...
    if gradesPerCandidate.forall((candidate, grades) => grades.isEmpty) then
      ...
      val bestMedianGrade: Grade =
        gradesPerCandidate.values
          .filter(_.nonEmpty)
          .map(gradeSeq => Grade.median(gradeSeq))
          .maxBy(_.ordinal)
      ...
      val bestCandidates: Map[Candidate, Seq[Grade]] =
        gradesPerCandidate.filter((candidate, gradeSeq) => gradeSeq.contains(bestMedianGrade))
      ...
      if bestCandidates.size == 1 then
        ...
        val bestCandidatesMinusOneMedianGrade: Map[Candidate, Seq[Grade]] =
          bestCandidates.map((candidate, gradeSeq) => candidate -> Set.from(gradeSeq).diff(Set.empty[Grade] + bestMedianGrade).toSeq)
