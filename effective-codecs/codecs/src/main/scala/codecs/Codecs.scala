...
trait DecoderInstances:
  ...
  given intDecoder: Decoder[Int] =
    Decoder.fromPartialFunction { case int : Json.Num if int.value.isValidInt => int.value.intValue /*decodeAs[Int].getOrElse(0)*/ }

  /** A decoder for `String` values */
  // TODO Define a given instance of type `Decoder[String]`
  given stringDecoder: Decoder[String] =
    Decoder.fromPartialFunction { case str: Json.Str => str.value /*decodeAs[String].getOrElse("")*/ }


  /** A decoder for `Boolean` values */
  // TODO Define a given instance of type `Decoder[Boolean]`
  given booleanDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction { case bool: Json.Bool => bool.value /*decodeAs[Boolean].getOrElse(false)*/ }
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def decodeAndPrepend(item: Json, previouslyDecodedItems: List[A]): Option[List[A]] =
      decoder.decode(item) match
        case Some(st) => Some(st +: previouslyDecodedItems)
        case None => None
    ...
    def processItem(item: Json, maybePreviouslyDecodedItems: Option[List[A]]): Option[List[A]] =
      if maybePreviouslyDecodedItems.isEmpty then None else decodeAndPrepend(item, maybePreviouslyDecodedItems.get)
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction{
      case json : Json.Obj =>  decoder.decode(json.fields.get(name).get)
      case _:Json => None
    }
